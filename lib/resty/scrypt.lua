...
local function calibrate(options)
    ...
    if (scrypt.calibrate(options.maxmem, options.memfrac, options.maxtime, n, r, p) == 0) then
...
local function crypt(options)
    ...
    if (type(options.salt) ~= "string") then
        ...
        options.salt = hex(random.bytes(options.salt_size))
    end
    ...
    if (scrypt.crypto_scrypt(options.secret, #options.secret, options.salt, #options.salt, n[0], r[0], p[0], b, options.key_len) == 0) then
